...
# A1 セルに付与する閲覧専用コメント (ホバー表示). A1 の値は曜日合計を占有するため、
# 「編集しても取り込まれない」注意はバナーではなくコメントで添える.
PFV_GRID_A1_COMMENT_TEXT: Final = (
    "※閲覧専用の静的スナップショット (export 時点)。\n"
    "編集しても取り込まれません。\n"
    "編集は『固定訪問パターン（編集用）』で行ってください。"
)


# ---------------------------------------------------------------------------
# Phase G-55: 静的グリッド集計シート (参照元「スケジュール枠組み（仮）」形式) のレイアウト
# ---------------------------------------------------------------------------
#
# 参照元シートは「コースごとの縦ブロック」を並べる. 1 ブロック = 6 日分 (月〜土),
# 1 日 = 5 列 [時間帯, 氏名, 住所, 複数, 条件]. 日ブロックは col 1,6,11,16,21,26 開始.
...
# サブ見出し (各日 5 列).
GRID_DAY_SUBHEADERS: Final[tuple[str, ...]] = ("時間帯", "氏名", "住所", "複数", "条件")
# 曜日名 (row1 col2 など). 月〜土.
GRID_WEEKDAY_FULL_LABELS: Final[tuple[str, ...]] = (
    "月曜日",
    "火曜日",
    "水曜日",
    "木曜日",
...
# 時刻スロット 09:30〜18:00 を 30 分刻み (18 スロット).
GRID_TIME_SLOTS: Final[tuple[tuple[int, int], ...]] = tuple(
    (h, m)
    for h in range(9, 19)
    for m in (0, 30)
    if not (h == 9 and m == 0) and not (h == 18 and m == 30)
)
