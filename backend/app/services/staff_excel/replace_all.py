...
def _parse_staff_row_replace(
    ...
    *,
    ...
    parsed["secondary_office_ids"] = []
    if not _is_blank(raw_secondary):
        ...
        for sc in sec_codes:
            ...
            if sec_office is None:
                skip_codes.append(sc)
                continue
            if sec_office.id in seen_ids:
                continue
            seen_ids.add(sec_office.id)
            sec_ids.append(sec_office.id)
        parsed["secondary_office_ids"] = sec_ids
        if skip_codes:
            logger.warning(
                "staff_excel replace_all row=%d: secondary_office 不明コード %r を skip",
                row_number,
                skip_codes,
            )
    ...
    if existing_obj is not None:
        ...
        # Phase E-7 (gap P0-2): secondary_offices は完全置換セマンティクス.
        # 全件物理削除 → Excel 値を新規 INSERT する.
        update_dict["_secondary_office_ids"] = list(parsed.get("secondary_office_ids") or [])
        return (
            StaffExcelImportRow(
                row_number=row_number,
                staff_id=existing_obj.id,
    ...
    if resurrect_target is not None:
        ...
        return (
            ...
            ),
            {
                ...
                "_updates": updates,
                # Phase E-7 (gap P0-2): 完全置換セマンティクス.
                "_secondary_office_ids": list(parsed.get("secondary_office_ids") or []),
            },
        )
    ...
    # Phase E-7 (gap P0-2): secondary_offices.
    new_dict["_secondary_office_ids"] = list(parsed.get("secondary_office_ids") or [])
    changes_for_view = [
        StaffExcelChange(field=k, old_value=None, new_value=_serializable(new_dict[k]))
        for k in sorted(new_dict.keys())
        if not k.startswith("_") and k not in ("id",)
    ]
    if new_dict["_secondary_office_ids"]:
        changes_for_view.append(
            StaffExcelChange(
...
# ---------------------------------------------------------------------------
# Override row parser (Phase E-7 gap P1, replace-all semantics)
# ---------------------------------------------------------------------------


def _parse_override_row_replace(
    row_number: int,
    row: tuple[Any, ...],
    *,
    existing_staff: dict[UUID, Staff],
    ...
    if weekday is None:
        return (
            OverrideExcelImportRow(
                ...
                iso_week=iso_week,
                operation="error",
                error_message=f"weekday の値が候補外: {wd_str!r}",
            ),
            None,
        )
    ...
    if override_type not in OVERRIDE_TYPE_VALUES:
        return (
            OverrideExcelImportRow(
                ...
                weekday=weekday,
                operation="error",
                error_message=f"override_type の値が候補外: {override_type!r}",
            ),
            None,
        )
    ...
    if key in pending_new_keys:
        return (
            OverrideExcelImportRow(
                ...
                weekday=weekday,
                operation="error",
                error_message="同ファイル内で (staff_id, iso_year, iso_week, weekday) が重複しています",
            ),
            None,
        )
    pending_new_keys.add(key)
    ...
    return (
        OverrideExcelImportRow(
            ...
            operation="new",
            changes=[
                StaffExcelChange(field="override_type", old_value=None, new_value=override_type),
                StaffExcelChange(field="start_time", old_value=None, new_value=start_str),
                StaffExcelChange(field="end_time", old_value=None, new_value=end_str),
            ],
        ),
...
# ---------------------------------------------------------------------------
# Edit-sheet parsers (Phase G-56: 1 行化, replace-all は全件 INSERT のみ)
# ---------------------------------------------------------------------------


def _resolve_staff_for_edit_row_replace(
    raw_id: Any,
    raw_code: Any,
    *,
    existing_staff: dict[UUID, Staff],
    ...
    existing_code_to_id: dict[str, UUID],
    resurrect_staff_ids: set[UUID],
) -> tuple[UUID | None, str | None, Staff | None, str | None]:
    staff_id: UUID | None = None
    try:
        if not _is_blank(raw_id):
            staff_id = _read_uuid(raw_id)
    except ValueError:
        return None, _read_str(raw_code), None, f"staff_id が UUID 形式ではありません: {raw_id!r}"

    staff_code = _read_str(raw_code)
    if staff_id is None:
        if not staff_code:
            return None, None, None, "staff_id / staff_code のどちらも空です"
        if staff_code in existing_code_to_id:
            staff_id = existing_code_to_id[staff_code]
        elif staff_code in new_code_to_uuid:
            staff_id = new_code_to_uuid[staff_code]
            ...
            return (
                ...
                staff_code,
                None,
                f"staff_code が DB / 同ファイル内に存在しません: {staff_code!r}",
            )

    is_pending = staff_id in set(new_code_to_uuid.values()) or staff_id in resurrect_staff_ids
    staff_obj: Staff | None = existing_staff.get(staff_id) if not is_pending else None
    if staff_obj is None and not is_pending:
        return staff_id, staff_code, None, f"staff_id が DB に存在しません: {staff_id}"
    staff_code_for_view = staff_obj.code if staff_obj is not None else staff_code
    return staff_id, staff_code_for_view, staff_obj, None


def _parse_shift_edit_row_replace(
    row_number: int,
    row: tuple[Any, ...],
    *,
    existing_staff: dict[UUID, Staff],
    ...
    existing_code_to_id: dict[str, UUID],
    resurrect_staff_ids: set[UUID],
    seen_staff_ids: set[UUID],
) -> tuple[list[ShiftExcelImportRow], list[dict[str, Any]]]:
    """Phase G-56: 「勤務シフト（編集用）」1 行 (replace-all). 7 曜日全件 new INSERT.

    開始&終了両方あり → is_on=True + 時刻. 両方空欄 → is_on=False (休, 時刻なし).
    """
    cells: dict[str, Any] = {
    ...
    if is_sample_row(cells.get("staff_code"), cells.get("staff_id")):
        return [], []
    staff_id, staff_code_for_view, _staff_obj, err = _resolve_staff_for_edit_row_replace(
        cells.get("staff_id"),
        ...
        resurrect_staff_ids=resurrect_staff_ids,
    )
    if err is not None:
        return (
            [
        ...
        return (
            [
                ShiftExcelImportRow(
                    ...
                    staff_code=staff_code_for_view,
                    operation="error",
                    error_message="同ファイル内で同一スタッフが複数行に現れています",
                )
            ],
            [],
        )
    ...
    diff_rows: list[ShiftExcelImportRow] = []
    ops: list[dict[str, Any]] = []
    errors: list[str] = []
    specs: dict[int, dict[str, Any]] = {}
    for wd_key, wd_int in SHIFT_EDIT_WEEKDAY_KEY_TO_INT.items():
        try:
            start_str = _read_hhmm(cells.get(f"{wd_key}_start"))
        except (ValueError, TypeError) as exc:
            errors.append(f"{wd_key} の開始時刻が HH:MM 形式ではありません: {exc}")
            continue
        try:
            end_str = _read_hhmm(cells.get(f"{wd_key}_end"))
        except (ValueError, TypeError) as exc:
            errors.append(f"{wd_key} の終了時刻が HH:MM 形式ではありません: {exc}")
            continue
        if start_str is not None and end_str is not None:
            specs[wd_int] = {
                "is_on": True,
                "start_time": _hhmm_to_time(start_str),
                "end_time": _hhmm_to_time(end_str),
                "start_str": start_str,
                "end_str": end_str,
            }
            ...
            errors.append(f"{wd_key} は開始・終了の両方、または両方空欄にしてください")
    if errors:
        return (
            [
                ShiftExcelImportRow(
    ...
    for wd_int in range(7):
        ...
        diff_rows.append(
            ShiftExcelImportRow(
                ...
                operation="new",
                changes=[
                    StaffExcelChange(field="is_on", old_value=None, new_value=spec["is_on"]),
                    StaffExcelChange(
                        field="start_time", old_value=None, new_value=spec["start_str"]
                    ),
                    StaffExcelChange(field="end_time", old_value=None, new_value=spec["end_str"]),
                ],
            )
        )
...
def _parse_override_edit_row_replace(
    row_number: int,
    row: tuple[Any, ...],
    *,
    existing_staff: dict[UUID, Staff],
    ...
    existing_code_to_id: dict[str, UUID],
    resurrect_staff_ids: set[UUID],
    seen_week_keys: set[tuple[UUID, int, int]],
) -> tuple[list[OverrideExcelImportRow], list[dict[str, Any]]]:
    """Phase G-56: 「勤務例外（編集用）」1 行 (replace-all). 各曜日例外を new INSERT."""
    cells: dict[str, Any] = {
        col_key: (row[idx] if idx < len(row) else None)
    ...
    try:
        iso_year = _read_int(cells.get("iso_year"))
        iso_week = _read_int(cells.get("iso_week"))
    except (ValueError, TypeError):
        return (
            [
                OverrideExcelImportRow(
                    ...
                    staff_code=staff_code_for_view,
                    operation="error",
                    error_message="iso_year / iso_week が整数ではありません",
                )
            ],
            [],
        )
    ...
    week_key = (staff_id, iso_year, iso_week)
        ...
        return (
            [
                OverrideExcelImportRow(
                    ...
                    iso_week=iso_week,
                    operation="error",
                    error_message="同ファイル内で同一 (staff, iso_year, iso_week) が複数行に現れています",
                )
            ],
            [],
        )
    ...
    reason = _read_str(cells.get("reason"))
    diff_rows: list[OverrideExcelImportRow] = []
    ops: list[dict[str, Any]] = []
    errors: list[str] = []
    for wd_key, wd_int in OVERRIDE_EDIT_WEEKDAY_KEY_TO_INT.items():
        try:
            parsed = parse_override_cell(cells.get(wd_key))
        except ValueError as exc:
            errors.append(f"{wd_key}: {exc}")
            continue
        if parsed is None:
            continue
        ot, start_str, end_str = parsed
        ops.append(
            {
                "_op": "new",
                "staff_id": staff_id,
                ...
                "weekday": wd_int,
                "override_type": ot,
                "start_time": _hhmm_to_time(start_str) if start_str else None,
                "end_time": _hhmm_to_time(end_str) if end_str else None,
                "reason": reason,
            }
        )
        diff_rows.append(
            OverrideExcelImportRow(
                ...
                operation="new",
                changes=[
                    StaffExcelChange(field="override_type", old_value=None, new_value=ot),
                    StaffExcelChange(field="start_time", old_value=None, new_value=start_str),
                    StaffExcelChange(field="end_time", old_value=None, new_value=end_str),
                ],
            )
...
async def parse_and_diff_replace_all(
    ...
    *,
    ...
    # Phase G-56: 新「勤務シフト（編集用）」優先, 無ければ旧 per-row へフォールバック.
    use_shift_edit_sheet = SHEET_SHIFT_EDIT in wb.sheetnames
    if not use_shift_edit_sheet and SHEET_SHIFT not in wb.sheetnames:
        raise ValueError(f"シート「{SHEET_SHIFT_EDIT}」が見つかりません")
    use_override_edit_sheet = SHEET_OVERRIDE_EDIT in wb.sheetnames
    ...
    resurrect_ids = set(resurrect_code_to_uuid.values())
    if use_shift_edit_sheet:
        ws_f = wb[SHEET_SHIFT_EDIT]
        seen_staff_ids: set[UUID] = set()
        for r_idx, row in enumerate(ws_f.iter_rows(min_row=2, values_only=True), start=2):
            if _row_is_empty(row):
                continue
            diff_rows_edit, ops_edit = _parse_shift_edit_row_replace(
                r_idx,
                row,
                existing_staff=existing_staff,
                new_code_to_uuid=new_code_to_uuid,
                existing_code_to_id=existing_code_to_id_with_resurrect,
                resurrect_staff_ids=resurrect_ids,
                seen_staff_ids=seen_staff_ids,
            )
            shift_rows.extend(diff_rows_edit)
            shift_ops.extend(ops_edit)
    else:
        ws_f = wb[SHEET_SHIFT]
    ...
    if use_override_edit_sheet:
        ...
        for r_idx, row in enumerate(ws_o.iter_rows(min_row=2, values_only=True), start=2):
            ...
            diff_rows_edit, ops_edit = _parse_override_edit_row_replace(
                ...
                existing_code_to_id=existing_code_to_id_with_resurrect,
                resurrect_staff_ids=resurrect_ids,
                seen_week_keys=seen_week_keys,
            )
            override_rows.extend(diff_rows_edit)
            override_ops.extend(ops_edit)
    elif SHEET_OVERRIDE in wb.sheetnames:
        ws_o = wb[SHEET_OVERRIDE]
        pending_override_keys: set[tuple[UUID, int, int, int]] = set()
        for r_idx, row in enumerate(ws_o.iter_rows(min_row=2, values_only=True), start=2):
            if _row_is_empty(row):
                continue
            diff_row, op = _parse_override_row_replace(
                r_idx,
                row,
                existing_staff=existing_staff,
                new_code_to_uuid=new_code_to_uuid,
    ...
    # Phase E-7 (gap P1): 既存 override count.
    override_count = (
        await db.execute(
            select(func.count())
            .select_from(StaffWeeklyOverride)
            .where(
                StaffWeeklyOverride.staff_id.in_(select(Staff.id).where(Staff.deleted_at.is_(None)))
            )
        )
    ).scalar_one()
    ...
    for r in override_rows:
        match r.operation:
            case "new":
                summary.override_to_create += 1
            case "error":
                summary.override_error += 1
    summary.override_to_replace = int(override_count or 0)
...
async def apply_replace_all(
    ...
    *,
    ...
    順序:
      1. 削除対象 staff の shift / secondary / override を物理削除
      2. 削除対象 staff を soft delete
      3. 残り alive staff の shift / secondary / override を全件物理削除
      4. staff_ops apply
      5. shift_ops INSERT
      6. secondary_office_ids INSERT (Phase E-7 gap P0-2)
      7. override_ops INSERT (Phase E-7 gap P1)
    """
    if staff_ids_to_soft_delete:
        ...
        for sh in shifts_for_delete:
            await db.delete(sh)
        # Phase E-7 (gap P0-2 / P1): cascade で消えるが明示削除で順序安定.
        sec_for_delete = (
            await db.scalars(
                select(StaffSecondaryOffice).where(
                    StaffSecondaryOffice.staff_id.in_(staff_ids_to_soft_delete)
                )
            )
        ).all()
        for so in sec_for_delete:
            await db.delete(so)
        ovs_for_delete = (
            await db.scalars(
                select(StaffWeeklyOverride).where(
                    StaffWeeklyOverride.staff_id.in_(staff_ids_to_soft_delete)
                )
            )
        ).all()
        for ov in ovs_for_delete:
            await db.delete(ov)
        await db.flush()
    ...
    if alive_staff_ids:
        ...
        for sh in rest_shifts:
            await db.delete(sh)
        # Phase E-7 (gap P0-2): 既存 secondary_offices 全件物理削除.
        rest_sec = (
            await db.scalars(
                select(StaffSecondaryOffice).where(
                    StaffSecondaryOffice.staff_id.in_(list(alive_staff_ids))
                )
            )
        ).all()
        for so in rest_sec:
            await db.delete(so)
        # Phase E-7 (gap P1): 既存 overrides 全件物理削除.
        rest_ovs = (
            await db.scalars(
                select(StaffWeeklyOverride).where(
                    StaffWeeklyOverride.staff_id.in_(list(alive_staff_ids))
                )
            )
        ).all()
        for ov in rest_ovs:
    ...
    # Phase E-7 (gap P0-2): apply 後に secondary_offices を INSERT するため記録.
    secondary_office_inserts: list[tuple[UUID, list[UUID]]] = []
    for op in staff_ops:
        op_type = op.get("_op")
        if op_type == "new":
            data = {
            ...
            db.add(Staff(**data))
            sec_ids = op.get("_secondary_office_ids") or []
            if sec_ids:
                secondary_office_inserts.append((op["_assigned_id"], sec_ids))
            ...
            sec_ids = op.get("_secondary_office_ids") or []
            if sec_ids:
                secondary_office_inserts.append((sid, sec_ids))
        elif op_type == "update":
            sid = op["_staff_id"]
            if sid not in staff_by_id:
                staff_by_id[sid] = await db.get(Staff, sid)
    ...
    # Phase E-7 (gap P0-2): secondary_offices INSERT.
    for sid, sec_ids in secondary_office_inserts:
        for off_id in sec_ids:
            db.add(StaffSecondaryOffice(staff_id=sid, office_id=off_id))
    if secondary_office_inserts:
        await db.flush()

    # Phase E-7 (gap P1): override_ops INSERT (replace-all は new のみ).
    if override_ops:
