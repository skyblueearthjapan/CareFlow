...
11. **`/records` の拠点フィルタと職員の異動**: `office_id` は「行の `office_id`」と「録音者の**現在の** `staff.primary_office_id`」のどちらかで当てる（Phase 2-B 実装）。このため職員が異動すると**その職員の過去の録音まで新しい拠点の一覧に移る**（患者が付いた録音は行の `office_id` で旧拠点からも引ける）。「異動前の記録は旧拠点に残す」が正なら、録音時の所属を行に持つか第 2 レグを落とす。現状は据え置き。
