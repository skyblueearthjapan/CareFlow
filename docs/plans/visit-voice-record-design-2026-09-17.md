...
**決定（2026-09-17・今泉様）**: 処理は **Vertex AI 東京リージョン（asia-northeast1）で完結**させる。文字起こし = **Gemini 3.5 Flash-Lite**、要約 = **Gemini 3.8 Flash**。国外処理（OpenAI 等）とローカル LLM は採らない（理由 §1-h）。AI Studio 版 Gemini は規約上不可。プロバイダは設定で切り替え可能な構造にし、将来の見直し余地を残す。
