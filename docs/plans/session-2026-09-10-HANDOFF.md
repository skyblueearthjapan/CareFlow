# セッション引き継ぎ 2026-09-10（★次セッションはまず `session-2026-09-11-HANDOFF.md`）（患者ステータス×予定の連動: Phase 1 デプロイ → Phase 2 入口ガード → Phase 3 表示の保険・突合）
...
- **サービス内容 21 対の正体（9/11）**: カイポケ側で「職員1の職種」と「サービス内容の正看/准看」が不整合（A=高岡(准看)なのに正看 14 件 / B=正看なのに准看 7 件・全て単独訪問）。実績側にも既に 14 件（9/7〜9/10）→ 請求区分の誤り。らく助側の値は 21 件とも正しい。**根本原因（らく助側の不具合）**: 担当変更で資格が変わる差分を diff engine が `_service_matches` の類似判定で `edit` にし、RPA edit はサービス内容を変えられない（`kaipoke-service-content-design.md` §3-1）→ 担当・職種だけ変わりサービス内容が残る（9/3 送信の 齋藤/岡村/園田/河野/藤江/武藤 が該当）。手作業の担当変更でも同じ。**対処**: 手順書 `docs/reports/2026-09-11-w37-service-content-fix-procedure.html`（カイポケで 21 件手修正）。**バックログ**: (a) diff engine: 資格（正看/准看）が変わる担当変更は edit ではなく delete+add にする (b) 予実比較レポートに「資格不整合」判定（予定/実績とも・職種列は CSV にある） (c) 残タスク⑦ 正看優先ルール。
- **資格×請求区分の不整合 月次棚卸（実績 CSV・9/10 21:34 取得分）**: 6 月 0 / 7 月 0 / 8 月 0 / **9 月 18**（W36 4 件 = 藤原 9/3・鈴木乃愛 9/3・唐鎌 9/4・伊藤 9/5、全て 高岡(准看)×正看・らく助送信の記録なし＝手作業起因 / W37 14 件）。らく助起因と特定できるのは 9/3 送信の edit 6 件（齋藤 9/8・岡村/園田/河野 9/10・藤江/武藤 9/11）。手順書を 25 件（実績 18 + 予定 7）に更新。
- **残り 18 件の起因（9/11 夕・手修正後の再比較 41df1ff8）**: 予定 17 件（9/15〜9/17）は全て らく助 9/8 12:05 送信（job 0ff737af）の内容と担当が異なる（例: 篠原 9/16 09:00 らく助送信=高岡・准看 → 現況=川名・准看 / 唐鎌 9/15 らく助送信=宇田川 16:55・正看 → 現況=高岡 16:10・正看）。9/8 以降 W38 の送信ジョブは無い → カイポケ画面での手作業の担当変更（サービス内容は追従しない）が起因。らく助側の値は 17 件とも資格と整合。実績 1 件（中原 9/11 14:00 熊澤・准看）も らく助の記録は 13:00 高岡・准看（整合）で、カイポケ側で 14:00 熊澤へ変更された行の実績 → らく助起因ではない。対処 = 手修正（実績 1 + 予定 17）または W38 取込→送信（修正後の grade_change で自動是正）。
- **取込（カイポケ→らく助）の監査（9/11 夜・PO 質問「取込は問題ないか」）**: 正看/准看は問題なし＝取込はサービス内容/職種列を読みも書きもしない（`inbound.py`/`replace_inbound.py` に参照ゼロ・`local_diff.py:186` で inbound は grade 判定オフ）。担当・日時だけ取り込み、区分は送信時に資格から決まる → カイポケ側の不整合行を取り込んでも次の送信で是正（W38 机上 diff で 17 件とも一致を確認）。**バックログ（未修正）**: (1) smart-inbound の date_change が置換日→打刻日をまたぐと訪問が消える（結果行も出ない・`inbound.py:879-890` の before 側日付フィルタ）。逆方向は autoflush=False で wipe が移動行を見落とし UNIQUE 409 で取込全体が失敗し得る。(2) **新規**: 月跨ぎ週の差分パート（打刻あり日）は `build_local_diff(month=week_start の月)` + engine の wrap フィルタ（day>=start or day<=end）で、らく助側に**前月 1〜N 日の行**が混入し翌月 1〜N 日と比較される（`integrations.py:4567-4575`・`engine.py:545-548`）。置換パートは安全。次の該当週 = 9/28〜10/4 → それまでに修正。(3) 仕様注意: 置換は wipe-first（名寄せ不可/担当なし/非稼働/重複/時刻不正はスキップ＝らく助から消える・通知のみ）、青ピン/⭐/manual も保護されない、日曜は smart 対象外、職員3 は落ちる、同一患者同時刻 2 行は 1 行に潰れる、コース担当付替/臨時コース削除・新設、undo は kaipoke_service_override と is_unplanned を復元しない（本番 override は 0 件）。
- **取込 2 件の根治 → 本番デプロイ済み（9/11 23:30 JST・a81c1c8・backup `backups/pre-deploy-inbound-fix-20260911-1429.sql.gz`・migration 無し）**: (1) smart-inbound の跨ぎ date_change（`_in_day_scope`・flush・`_smart_held_replace_days`＝未適用/未選択/日曜宛ては元日を置換から外す・打刻付き訪問の移動は 422）(2) 月跨ぎ週（`_keep_week_rows` で両側を月ごとに週内日へ制限・カイポケ側は両月 export → 拠点フィルタ後に週 snapshot・月 snapshot は保存しない）+ 送信 `/apply` の month は選択行の実日付から決定（2 か月にまたがる選択は 422・9/30→10/1 の date_change は `month_boundary` で除外）。レビュー（BLOCKER1/HIGH3/MEDIUM4/LOW5）是正済み。本番机上検証: W40 diff の らく助側 138 行（9/1〜9/4 の 100 行は不混入）・months=[2026-09, 2026-10]。**残バックログ**: (a) 月跨ぎ週の smart-preview / diff-inbound は export 2 本（~90 s×2）で Cloudflare 100 秒制限に当たり得る → ジョブ化 or 直前の週 snapshot 再利用（🔄突合は ②smart-preview が保存した週 snapshot を ③diff-inbound で使えば export 0 本）。9/28 週の突合前に対応が望ましい (b) 置換で消える訪問に紐づく ⭐placed_visit_id / 直接同行リンクの付け替え (c) 手動 /apply-inbound の days 指定は before 側のみ（仕様として明記済み）。（→ 根治済み: §10・らく助 13af578 / RPA 0e00107。）

## 10. 請求区分（正看/准看）不整合の根治（9/11 午後・らく助 13af578 / RPA 0e00107）
- **方式**: 分割しない。現況/最適化の `service_grade` が異なる edit/date_change に `grade_change=True`・`service_type`=らく助値・`service_type_from`=カイポケ値を付与し 1 件で送る。「…・正看」対「…・准看」で Pass1-3 に乗らない残り行も **Pass 3.5** で (同日・同利用者・同開始) を対にして 1 件の edit に統合（従来は対になっていない delete+add＝8/31 消失パターン。9/7 週の 21 件はこの形）。S3 ゲート `KAIPOKE_RPA_SERVICE_BRANCH_ENABLED` が閉なら従来どおり（本番は True）。inbound 不変。未送信/一覧/payload/⇧上書き逆変換で同じ helper。
- **RPA**: `Correction.grade_change/service_type_from`（`correction_from_dict` で未知キー無視）。edit で grade_change なら `_apply_move_with_reorder`（削除を検証→新サービス内容で再追加→失敗時は `service_type_from` で復元・reason `grade_change_rollback`／元が無ければ `grade_change_rollback_no_source` で停止）。再試行時は削除完了マーカーで追加から再開（二重削除しない）。
- **デプロイ順**: RPA 先（0e00107 反映済み 12:xx）→ らく助 13af578。BE は payload に 2 キーを常時送るため、旧 RPA だと `Correction(**item)` で TypeError → 順番厳守。
- **レビューで捕まえた重大事項**: 初版の delete+add 分割は RPA が別々に処理し失敗時に削除だけ残る（8/31 型）／時刻変更行の再追加が旧サービス内容を送っていた／S3 ゲート閉のとき edit がガードを素通り／⇧上書き経路と未送信にフラグが無い。
- **残（バックログ）**: 区分だけが違い staff/時刻が同じ行（Kaipoke「精神基本療養費Ⅰ」suffix 無し vs らく助「…・准看」）は Pass 2 の has_diff=False で差分にならない＝らく助送信では直らない（予実比較「資格不整合」で検出は可能）。FE「請求区分変更」バッジ未実装。残タスク⑦（正看優先 同行ルール）。
