# 自動スタッフ割当 4段ソルバ（マネージャー動的動員＋拠点跨ぎ救援）設計書 v2.0

作成 2026-07-12 / ステータス: **v2.0 本番稼働 `2c93ef1`（2026-07-12・healthz内外OK・PO実機確認済み・高評価）**
v2.0 実績: BE layer3系 113 pass / FE 1199 pass 0 fail / tsc 0。critic REVISE 6 MAJOR・
ディレクター指摘（採用ガード退行防止）・code-reviewer MAJOR 1+MINOR すべて反映済み。
**§11 が最終仕様の正典**（§0〜§9 のQ3・rotation_relaxed 関連記述は v1.1 の履歴）。
