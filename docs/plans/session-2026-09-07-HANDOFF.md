...
| 8-6 | **⭐/プールカードの DnD を全ビューへ（PO 指示 9/8）**: 週ビュー（週全体）・コースの週ビュー・**職員スケジュール（必須）** でもドラッグ＆ドロップで配置できるようにする。挙動 = ①どこでも掴める ②置く瞬間に案内/警告 ③曜日が違う場所なら「これは◯曜日の予定ですが、ここに配置して本当によろしいですか？」の確認を出してから配置（掴めない/曜日タブ案内の現行 8-2 挙動は置き換え）。時間軸の無いビューでは開始時刻の指定が要る（ドロップ後の確認モーダルで曜日警告＋時刻選択＋コース表示→配置）。BE: ⭐ の曜日変更を伴う配置（mark.weekday 更新 or place に weekday 指定・同曜日の既存○との衝突は 409）。職員スケジュールはブラウザ標準 DnD のため受け口の追加が必要 | 調査中 |
