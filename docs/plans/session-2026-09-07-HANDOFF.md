...
## ★ 最初の 5 分
1. **状態**: **本番デプロイ済み HEAD `203a5c1`**（2026-09-08 夕・frontend 再作成・8-6 Phase 2 込み。前 = e0de2b4 backend+frontend（Phase 1）。前 = 34794eb 午前 frontend（8-1/8-2）。前回 = 0337525 9/7 22:xx backend+frontend。2 回目 = f5cbae9 可読性改修 / 1 回目 = fa53523 15:2x JST・backup `pre-deploy-20260907-0619.sql.gz` ほか同日分）。push 済み。本番 DB への書込は §1-3 の 5 件のみ（伊藤様の W38〜W40 訪問・バックアップ `pre-ito-placement-20260907-0344.sql.gz`）。
...
| 8-5 | 昨夜デプロイ分の実機確認 5 点（§7）＋ 9/8 分: (i) 職員スケジュールで ⭐/プールカードをセルへドロップ→確認モーダル（曜日違いなら警告）→配置 (j) 曜日違いの ⭐ を置くとカレンダーの ○ が移る (k) 標準 DnD のコース帯/訪問帯の移動が従来どおり動く (l) 週タイムライン/週リストへ ⭐/プールカードをドロップ→同曜日は即配置・曜日違いは確認・休の列は拒否トースト (f) ダイアログの 2 段セルと週合計内訳 (g) 同じ日に 2 回目の確認 (h) ⭐ カードを日タイムラインで同曜日の列へドラッグ→配置・他曜日は掴めない・クリックは従来ポップアップ | 未 |
| 8-6 | **⭐/プールカードの DnD を全ビューへ（PO 指示 9/8）**: 週ビュー（週全体）・コースの週ビュー・**職員スケジュール（必須）** でもドラッグ＆ドロップで配置できるようにする。挙動 = ①どこでも掴める ②置く瞬間に案内/警告 ③曜日が違う場所なら「これは◯曜日の予定ですが、ここに配置して本当によろしいですか？」の確認を出してから配置（掴めない/曜日タブ案内の現行 8-2 挙動は置き換え）。時間軸の無いビューでは開始時刻の指定が要る（ドロップ後の確認モーダルで曜日警告＋時刻選択＋コース表示→配置）。BE: ⭐ の曜日変更を伴う配置（mark.weekday 更新 or place に weekday 指定・同曜日の既存○との衝突は 409）。職員スケジュールはブラウザ標準 DnD のため受け口の追加が必要。設計 = `dnd-all-views-design-2026-09-08.md`。**BE 完了 cfaa42e**（place weekday・同一TX・409 構造化・早期 flush）。**FE Phase 1 完了 e0de2b4・レビュー承認・本番稼働**（sw-cell droppable・resolveDropTarget・PlacementConfirmDialog(曜日違い警告/コース/開始時刻・9:00〜18:00 ガード)・PlacementConflictConfirmDialog(409→既存○へ)・拠点違いは M 明示・ゲート撤去=どこでも掴める）。**Phase 2 完了 203a5c1・レビュー承認・本番稼働**（週タイムライン wtl-col: 同曜日は snap 時刻で即配置/曜日違いは確認・週リスト cwo-cell: 常に確認・休(定員0∧型なし∧訪問なし)は「開講していません」で拒否・確認モーダルに列の担当者名）。**追跡: BE 側の開講日バリデーション**（place-and-fix / place の course_template_id モードは閉じた曜日にも Course を作る＝いまは FE ガードのみ） | **完了・デプロイ済み** |
