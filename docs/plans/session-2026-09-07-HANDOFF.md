...
| # | 内容 | 状態 |
|---|---|---|
| 8-1 | **特別訪問週間ダイアログの表示整理**（PO 指示 9/8）: セルを上下 2 段（上=予定あり: 時刻/コース/担当のカード・下=追加枠）・○ を「**プール待ち（この日に 1 回追加・時間未定）**」/ ● を「配置済み HH:MM コース」/ 予定も枠も無い日は「空き」と表示・既に予定がある日に ○ を付けるときは「同じ日に 2 回目をプールに追加しますか？」の確認・週合計を「固定 N ＋ プール待ち M ＝ 合計（目標 T）」の内訳・目標超えは色・文言統一（＋=この日を保留プールに追加／○ メニュー=配置先を決める…(プールから盤面へ)・プールから外す／上部=「保留プールに N 件あります（時間未定）」／プール ⭐ カード=「特別訪問週間の追加枠・時間未定」）。高さ幅は広げてよい | 実装中（並行） |
| 8-2 | **⭐ チケットがプールでドラッグ＆ドロップできない**（PO 指摘 9/8）: 通常プール患者はカードを列へドラッグできるが ⭐ はクリック→候補提案のみ（8/31 調査 F-6）。BE `place` が course_template_id+start_time を受けるようになったので、通常のプールドロップと同じ経路で「列へ落とす＝place」にできるはず。調査→設計→実装 | 調査中（並行） |
