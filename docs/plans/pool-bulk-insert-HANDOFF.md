...
原因B=性別ブロック候補ゼロで review_item 不生成→過去の性別違反担当が黙って残留→
新設 unresolved_warnings（承認対象外・自動クリアなし）で可視化＋観測ログ
gender_blocked_no_candidate。割当ロジック本体は不変（レビューで検証）。
コミット `b9dae5f`。**運用注意: デプロイ後最初の自動スタッフ割当で残留違反が
ダイアログに現れる（新規発生ではなく過去からの残留の可視化）。担当変更で解消を。**
補助要因: 本番 active 66名中 sex_restriction 設定は14名 — 未設定患者の確認を現場に依頼中。

**W-10 完了（2026-07-04・本番稼働）**: 欠勤対応機能の廃止（本番 apply 0回・計算0回を実測→PO決定）。
StaffSubstituteDialog/エンジン等 約2,900行削除（`3e80cf8`＋参照除去 `19d9e70` — 
**コミット分割事故あり**: 1回目が削除のみで参照除去が漏れ、push後に発覚し追いコミットで完結。
本番到達前に修復。教訓=コミット直前の git status 全量確認）。
残置: manual_staff_override カラム・Layer3 保護フィルタ・保護テスト14本。
欠勤時の運用 = コースの「担当変更」（Ctrl+Z対応）。

**次回相談の予告（PO・2026-07-04）**: 「欠勤対応」の要否 — PO が「不要ではないか」と感触を表明
（詳細は後日）。相談時は W-8 と同じ進め方（実体調査→重複/固有価値分析→レポート→PO判断）で。

**W-9/W-9b 完了（2026-07-04・本番稼働）**: 週次ガイドを「週を生成」の右隣に対配置し、
ペアをツールバー**左端**（曜日タブの真上）へ — Row 1 は justify-between の両端配置に変更
（左=[週を生成][週次ガイド] / 右=[欠勤対応][新規患者登録][診断][最適化]｜[固定枠戻][全件保存]）。
「（5分の儀式）」等の儀式表現をユーザー可視文言から全廃（「約5分」の事実表記に）＋
マニュアル2種の旧称「自動スタッフ割付」15箇所を「自動スタッフ割当」に統一。
コミット `b75c58f` `90d0cbe`。
