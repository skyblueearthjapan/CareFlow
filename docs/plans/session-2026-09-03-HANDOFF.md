...
| 残差シート | `fd2bfc19`（diff-local 20:11 JST・29 件 = 送信可 25 + 担当なし 4） |
...
## 2. これからやること（復旧手順・PO 承認待ち）
**順序が重要**: 隠れ行を先に消さないと 9/9 の add が二重になる。
1. **隠れ行 8 件を RPA で削除**: payload = VPS `/tmp/hidden_delete_dry.json` の `dry_run` を false にして `POST /api/apply`（dry-run で 8 行とも「1 件のエントリを発見」を確認済み）。
2. **らく助から送信**: 盤面の 🔄差分最新化 → ⇧全件送る（担当なし 4 件は自動除外・25 件）。カイポケが軽い時間帯に。結果 details の `reason` を読む。
3. 🔄差分最新化 → 残差 = 都賀A 9/10 の 4 件（林・森田・松戸・石川）と麻生 9/9 だけになること。
4. **PO 判断**: 都賀A 9/10 の担当（らく助で付ければ次の送信で add される）／麻生様のカイポケ登録。
5. 8/31 の教訓どおり成否は export で確定（RPA の成功報告だけを信じない）。
