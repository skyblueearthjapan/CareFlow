...
| 6 | カイポケ月次CSV | **一般同行も職員名2に出力**（新人と同格）。職員名2/3 の割当は決定的順序（secondary → 同行[**support優先** → staff名昇順 → id] → mentor・staff_idデデュープ維持）。※support優先の理由: 一般同行=実働の2人目であり、育成目的の新人より優先枠に載せるのが実務上正しい（初版の「kind問わず名前昇順」は2026-08-17レビューHIGH-1を受けディレクター判断で改訂・実装`_entry_sort_key`と一致） |
