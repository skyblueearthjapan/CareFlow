...
- 発動条件: 通常候補0件 かつ 除外理由に時間起因（no_gap / no_pair_slot / travel_shortage）を含む
  ...
  - **W-15 で capacity_full を発動理由に追加**（計4件）。定員起因のときは方式b（+1名相談）の
    呼びかけと **並列表示**し（方式b → unblock の順・間に軽い区切り）、管理者が「ずらす」か
    「+1名」かを選べる。frees_capacity=true のプランには小バッジ「定員内に収まります」を表示。
- 結果表示: プランカード（上位5件）
...
- 分数上限×深さ2のテスト補強（1除去では分数が足りず2除去で開通するケース — W-15 レビュー LOW。経路自体は _fits 再評価で論理的に正しい）
