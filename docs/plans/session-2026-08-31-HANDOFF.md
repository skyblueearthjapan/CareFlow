...
- **追加（同日・デプロイ c58ff02）**: 折りたたみヘッダのボタン配置変更（PO スクショ指示）。1 段目 = ツール/週切替/実現性チェック/差分最新化/突合レポート＋右端に診断/最適化、**曜日タブ群（月〜土/週/職員スケジュール）は 2 段目左**・表示切替系は 2 段目右。実装 = `CourseDayTablePanel.tsx` の行区切り (`basis-full`) を曜日タブの前へ移動。展開時レイアウト不変・テスト 25 件 green。
- **追加（同日・デプロイ 5556883）**: PO 要望「差分最新化 → 突合レポートの流れ」→ 盤面に **`ReconcileToolbar` = 「🔄 差分最新化」＋「🔍 突合レポート」** を配置。最新化 = 連携ページ「この週の差分を計算」と同一の diff-local（約 1 分・完了トーストに差分件数・スナップショット更新）。連携ページは既存の差分計算ボタンがその役なので変更なし。

## 5-h. 対応済み（2026-09-01 午後・デプロイ a0edf16 / RPA b44808f）: 月跨ぎ週のイベント取込失敗
PO スクショ報告: 週 8/31〜9/5 の「カイポケから取り込む（訪問＋イベント）」が「指定日 2026-08-31 が表示週 2026-09-01〜2026-09-07 に含まれていません」でイベント側全滅（訪問側は取得済み）。
- **原因**: カイポケ職員スケジュール週間で前月末日 (8/31) を起点にすると、表示週が 9/1〜 に丸められることがある（間欠的・同日午後の再現試行では 8/31〜9/6 が正常表示された＝画面状態依存）。RPA `individual_tasks.py` は表示週チェックで即例外にしていた。
- **対策（多層防御）**: ① RPA (b44808f) = 表示週ズレ時に**翌日起点で再試行**し実際の `week_dates` を返す ② BE = `build_events_plan(covered_dates)` — 表示週に含まれない日は**削除判定から除外**（「取得できていない」を「カイポケに無い」と誤認して delete を出す事故の防止）し `uncovered_days` として応答 ③ FE = 確認ダイアログに「⚠ ◯/◯ はカイポケの表示週に含まれないため対象外」表示。
- **検証**: BE 27 件 / FE 16 件 green。本番実測 = 8/31 週プレビュー成功（fetched 64・週 8/31〜9/6 表示・今回は再試行不要・uncoveredDays=[]）。

## 5-i. 対応済み（2026-09-02・デプロイ 793cec0）: ドラッグ中に画面がスクロールせず画面外へ配置できない
PO 報告（職員スケジュールで上スクロール不可）。**調査結果 = 自動スクロール実装が皆無**で、PC は「ページ固定・ペイン内部スクロール」設計のためブラウザ任せでは全ドラッグ面（職員スケジュール/コース盤面/日タイムライン/週/プール/運転席）・全方向が不可だった。
- **実装**: `frontend/lib/hooks/useDragAutoScroll.ts` — document `dragover`（capture）でポインタ追跡 → rAF ループで `elementFromPoint` から祖先を内→外に走査し、スクロール可能コンテナの**可視領域の端 48px 圏内**で近いほど速く（最大 24px/フレーム）スクロール。縦横独立判定・どのコンテナも受け持たない軸はウィンドウへフォールバック・drop/dragend/dragover 途絶（300ms・ウィンドウ外検知）で停止。スケジュールページに 1 回常駐で全 DnD 面（今後の画面含む）をカバー。
- テスト 5 件（edgeDelta 純関数＋リスナー配線）green・typecheck/lint clean。実機はドラッグでの体感確認を PO に依頼。
