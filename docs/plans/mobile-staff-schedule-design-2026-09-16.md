...
## 7. 追補（同日 16:00）: W38 取込のダメ出しから決めた是正（お客様承認「全て実装」）
### D-1. 差分モードの臨時コース二重生成 → **バグではなかった（取り下げ）**
- 実装確認: `ensure_temp_course` は作ったコースを索引に登録し、同一実行内の後続 item は再利用する（同一拠点・同一職員の回帰テストを追加して固定）。置換モードも同様に再利用済み。
- 本番の 9/15 火 川名「臨」×2 の正体 = **拠点違い**（稲毛「臨」= 並木様、都賀「臨」= 手渡様。コースは拠点ごとなので正規の動作）。
- 残る小さな不整合（未修正）: dry-run（プレビュー）は臨時コースを作らないため、同一拠点でも 2 件目以降の明細が「臨時コース新設」と出て、実適用の「コース臨」と文言がずれる。
### D-2. 取込が作る同行リンクの `source`
- `Accompaniment.source` を `manual` → `import` に（取込由来の出所を正しく記録）。許容値の制約があれば追加。
### E-1. イベント取込の「手入力行の引き継ぎ（absorb）」
- 現状: `external_id` だけで突合し、手入力（manual/fixed・external_id NULL）と同内容でも add で二重になる。
- 変更: 追加前に **同一 staff × 同一 starts_at × 同一 title（trim・空白正規化）** の手入力行を探し、あれば `absorb`（行を `source='kaipoke'`・`external_id=key` に更新、ends_at/note はカイポケ値、`cancelled_at` は保持）。preview に `absorb` action と集計を追加。FE ラベル「引き継ぎ」。
- 一回限りの整理: `docs/tools/kaipoke-ops/dedupe_manual_events.py`（dry-run 既定・`--apply` で manual 側を除去・削除前に JSON 退避・manual 側だけの cancelled_at は kaipoke 側へ移す）。**実行は PO 承認後**。
### 3. 臨時コース 6 本（コードではなくマスタ）
- 実態: コース枠は拠点ごとの固定テンプレ。稲毛 = A〜E（5）+ M/M2、都賀 = A（1）+ M。人数に連動しない。都賀は 1 枠しか無いので 2〜4 人動く日は必ず「臨」になる（木曜 3 本）。稲毛は火曜の 2 本のうち 1 本が D-1 のバグ、もう 1 本（本名）は枠超過。
- 対応: 都賀のコーステンプレを実態（曜日別の最大人数）に合わせて増やす（管理画面のコーステンプレ）。コード変更なし。PO 判断。
