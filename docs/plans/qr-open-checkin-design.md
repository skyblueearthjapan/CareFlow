...
- `build_monitor` 拡張:
  ...
  - `actual_staff_*` は最新 **arrival** 基準のまま。arrival が 1 件も無い場合のみ
    最新 departure へフォールバック（表示が空になるより良い・M-3 実装時に追加）。
  - GET capability 分岐の 409/410 は **404 へ丸める**（存在秘匿・m-2。POST の 409/410
    意味論は不変。この結果、FE の qr フォールバック GET は別患者/失効を区別できず
    404 一括扱いになるが、POST 側の 409 分岐が代行/予定外導線を担うため実害なし）。
