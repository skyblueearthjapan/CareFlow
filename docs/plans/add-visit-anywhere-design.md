...
### 3-5. 画面サイズ・文字の基準（PO フィードバック 2026-09-07・本番初見「文字が小さく操作性が悪い。モーダルは大きくしてよい」）
調査で確定した初版の実態: 幅 672px（`max-w-2xl`）・本文の大半が 11px（18 箇所）と 10px（3 箇所）・見出し 14px・Input/select が 24〜28px・本体は画面高 68% の内部スクロール・全 5 セクションを縦 1 列。アプリの他の大型モーダルは 896〜1024px・画面高 90%。

| 項目 | 基準 |
|---|---|
| 幅・高さ | `max-w-5xl`（1024px）・`max-h-[90vh]`。狭い画面（<1024px）では 1 列に落ちる |
| 配置 | 2 列: 左 = ①患者・②日付・③時刻/所要/希望担当、右 = ④提案（日付ごとの行）・⑤反映先。フッタは固定 |
| 本文 | 14px（`text-sm`）を基準。セクション見出し 16px（`text-base` 太字）。タイトル 18px（ダイアログ既定） |
| 補足 | 12px（`text-xs`）は注記・バッジのみ。11px 以下は使わない |
| 入力・select | 高さ 36〜40px（`h-9`〜`h-10`）・文字 14px。既存 `Input`/`Button` の既定サイズに合わせる |
| ラジオ・チェック | ラベル全体をクリック領域にする行（高さ 32px 以上・余白 `py-1.5`）・入力 16px |
| 候補行 | 1 候補 = 1 行。コース名と担当を太字 14px、警告バッジは 12px |
| 結果ダイアログ | 幅 `max-w-3xl`・本文 14px・同じ基準 |
| E 確認モーダル | 幅 `max-w-2xl`・本文 14px（現状 12px 主体を引き上げ） |
