...
**表示の配置ルール（2026-07-12 PO実機フィードバック反映）**:
- カード内バッジ = **右上**（患者名と被らせない・ピン留めとはピン左へ退避）
- コース列ヘッダ = スタッフ名と**同一行**に配置（同行の有無でヘッダ高さを変えない）。
  週タイムライン=「👥名前」チップ／日タイムライン=**「👥」アイコンのみ**（selectの幅確保・titleで全文）
- **週リスト（CourseWeekOverview）** = 極小「👥」アイコンのみ（患者行右端／コース丸ごとはセル容量行右）・titleで名前
- **日リスト（TimelineDayList）** = 「👥名前」info色チップ（患者行＋コース見出し行）
- いずれも同行モード（active）中は非表示（選択UIはタイムライン専用）
