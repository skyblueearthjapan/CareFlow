...
test.describe('v2 Wave 15 スケジュール画面 (統合 1 画面) — W15 Phase 5 F-2', () => {
  // [Wave 16 Phase B] スケジュール UI 構造刷新によりこの spec 全体を skip。
  // 旧 ScheduleUnifiedView (コース×曜日マトリクス) は削除され、
  // 新 StaffWeekTablePanel (スタッフ別テーブル N 個) に置換された。
  // 完全削除はせず、後続 Wave で書き換え可能にするため skip ガードのみ。
  test.skip(true, '[Wave 16 Phase B] Schedule UI 構造刷新 (StaffWeekTablePanel)');
