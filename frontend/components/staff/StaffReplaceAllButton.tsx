...
import { replaceAllStaffExcel } from '@/lib/api/staffExcel';
import type { StaffExcelReplaceAllResponse } from '@/lib/schemas/staffExcel';
...
/**
 * スタッフマスタの完全置換 (バックアップ復元) ボタン.
 *
 * RBAC: admin のみ (parent でガード推奨).
 */
export function StaffReplaceAllButton() {
  ...
  const [preview, setPreview] = useState<StaffExcelReplaceAllResponse | null>(null);
  ...
  async function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      const result = await replaceAllStaffExcel({
        file,
        dryRun: true,
        accessToken,
        refreshToken,
  ...
  async function handleConfirmApply() {
    ...
    try {
      ...
      toast.success(
        `スタッフマスタを完全置換しました (` +
          `更新 ${summary?.staff_to_update ?? 0} / ` +
          `新規 ${summary?.staff_to_create ?? 0} / ` +
          `削除 ${summary?.staff_to_soft_delete ?? 0})`,
      );
        ...
        <DialogContent>
          ...
          <div className="space-y-2 text-sm text-text-secondary">
            <ul className="list-disc space-y-1 pl-5">
              <li>
                現状の全スタッフを Excel の内容で<strong>完全に置き換え</strong>ます
              </li>
              <li>
                Excel に無いスタッフは<strong>削除</strong>されます (soft delete)
              </li>
              <li>
                Excel の空セルは <strong>NULL</strong> で上書きされます
              </li>
              <li>勤務シフトは全件物理削除され、Excel から再投入されます</li>
              <li>
          ...
          {preview && summary ? (
            <div className="space-y-3 text-sm">
              <ul className="space-y-1">
                <li>
                  更新: <strong>{summary.staff_to_update}</strong> 件
                </li>
                <li>
                  新規: <strong>{summary.staff_to_create}</strong> 件
                </li>
                <li>
                  削除: <strong>{summary.staff_to_soft_delete}</strong> 件 (Excel
                  に無い既存スタッフ)
                </li>
                <li>
                  勤務シフト: 既存 {summary.shift_to_replace} 件を物理削除 → Excel から{' '}
                  {summary.shift_to_create} 件を再投入
                </li>
                {hasErrors ? (
              ...
              {preview.staff_to_soft_delete_preview.length > 0 ? (
                <div className="space-y-1 border-t border-border-default pt-3">
                  <p className="font-medium text-text-primary">
                    削除されるスタッフの一覧 ({preview.staff_to_soft_delete_preview.length} 件):
                  </p>
                  <div className="max-h-48 overflow-auto rounded border border-border-default bg-bg-muted/30 p-2">
                    <ul className="space-y-0.5 text-xs">
                      {preview.staff_to_soft_delete_preview.map((s) => (
                        <li key={s.staff_id} className="tnum">
                          {s.staff_code ?? '(no code)'} {s.name}
                        </li>
                      ))}
                    </ul>
                  </div>
