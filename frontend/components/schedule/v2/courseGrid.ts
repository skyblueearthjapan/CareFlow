...
export interface CourseGridVisit {
   ...
   *
   * 同 (患者, 曜日, スロット) に固定枠が存在するが、開始時刻が今週の実配置と
   * **一致しない** ときだけ、その固定枠の開始時刻 'HH:MM' が入る。
   * 一致しているとき / 固定枠が無いとき (= 希望訪問スケジュール由来) は null。
   *
   * 用途は 2 つ:
   *   1. 盤面でズレを可視化する (左端の帯 + 時刻行への併記)。
   *   2. ピン留めできない理由を正確に伝える。旧実装は固定枠が存在しても
   *      「先に固定枠登録が必要」と表示しており、嘘になっていた。
   */
