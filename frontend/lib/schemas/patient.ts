 ...
 *   (`dict | None`). The form captures `weekly_pattern` as a JSON string and
 *   parses it on submit; `special_week` is also a structured dict.
 */
import { z } from 'zod';
...
export function patientReadToFormValues(p: PatientRead): PatientFormValues {
  return {
    ...
    note: p.note ?? '',
    weekly_pattern: p.weekly_pattern ? JSON.stringify(p.weekly_pattern) : '',
